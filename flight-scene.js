...
(() => {
  ...
  function body(sections, offset, color, sides = 20) {
  ...
  body([[-4.8,.035,.035],[-3.8,.24,.25],[-1.8,.49,.46],[1.9,.51,.47],[2.8,.40,.40],[3.8,.19,.23],[4.2,.02,.025]],[0,0,0],[196,213,225]);
  ...
  for(const side of [-1,1]){
    foil([[1,-.13,side*.3],[-1.8,-.08,side*5.7],[-2.5,-.08,side*5.65],[-1.5,-.13,side*.3]],.055,[159,183,205]);
    foil([[-3,.13,side*.15],[-4.1,.2,side*2.15],[-4.65,.2,side*2.1],[-4.35,.13,side*.15]],.035,[173,198,215]);
    face([[-1.8,-.08,side*5.7],[-2.1,.48,side*5.83],[-2.55,.44,side*5.8],[-2.5,-.08,side*5.65]],[84,168,209]);
    body([[-2.9,.1,.1],[-2.55,.29,.29],[-1.3,.3,.3],[-1.13,.25,.25]], [0,.2,side*.78],[146,172,195],16);
    ...
    for(let x=-1.5;x<1.9;x+=.48) face([[x,.15,side*.465],[x+.23,.15,side*.465],[x+.23,.32,side*.411],[x,.32,side*.411]],[25,62,83]);
    face([[2.2,-.08,side*.47],[-2.7,-.08,side*.37],[-3.5,.02,side*.28],[-2.6,.02,side*.39],[2.2,.02,side*.48]],[53,136,177]);
  }
  foil([[-3.1,.2,0],[-3.9,2,0],[-4.5,2,0],[-4.5,.15,0]],.025,[64,140,179]);
  let width=0,height=0,yaw=-.48,pitch=.30,drag=null,paused=reduced.matches,visible=true,raf=0,last=0,phase=0;
  ...
  function loop(t){raf=0;if(!visible||document.hidden||paused||drag)return;if(t-last>32){phase+=.012;yaw=-.48+Math.sin(phase)*.16;pitch=.30+Math.sin(phase*.7)*.045;draw();last=t;}raf=requestAnimationFrame(loop);}
  ...
  document.getElementById('flight-reset').addEventListener('click',()=>{yaw=-.48;pitch=.30;phase=0;draw();});
  canvas.addEventListener('pointerdown',e=>{drag={x:e.clientX,y:e.clientY,yaw,pitch};canvas.setPointerCapture(e.pointerId);stop();});
  canvas.addEventListener('pointermove',e=>{if(!drag)return;yaw=drag.yaw+(e.clientX-drag.x)*.006;pitch=Math.max(-.6,Math.min(.9,drag.pitch+(e.clientY-drag.y)*.004));draw();});
